...
"""
im = Image.open("Affichage/openstreetmap.png")
logo = ImageTk.PhotoImage(im, master=canvas)
logo1 = canvas.create_image(0, 0, anchor='nw', image=logo)
"""
...
for i in range(len(points_x) - 1):
  for j in range(len(points_y) - 1):
    nombre_animaux_zone = len(
      gestion.nomCoordonnees(connection, conversion_pxx_coord(points_x[i]),
...
def AfficherCarte(AnimalSelect):
...
btn1 = Button(fenetre,
              ...
              relief=FLAT)  # création bouton 1

btn1.config(command=AfficherCarte(AnimalSelect))
